...
/* This is for reading. I don't think we read anything. */
static void mydmx_read_bulk_callback(struct urb *urb)
{
	struct usb_mydmx *dev;
...
static int mydmx_do_read_io(struct usb_mydmx *dev, size_t count)
{
	int rv;
...
static ssize_t mydmx_read(struct file *file, char *buffer, size_t count,
			  loff_t *ppos)
{
	struct usb_mydmx *dev;
	int rv;
...
retry:
	...
	if (dev->bulk_in_filled) {
		 ...
		 */
		if (available < count)
			mydmx_do_read_io(dev, count - chunk);
	} else {
...
/* This is for writing. We need this. */
